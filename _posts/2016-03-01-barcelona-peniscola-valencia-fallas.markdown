---
layout: post
title:  "Recorrido por Valencia en fallas"
subtitle:  "Barcelona - Peñíscola - Valencia"
date:   2016-03-01 00:00:00
last_modified_at:  2016-03-01 00:00:00
excerpt: "Recorrido por Valencia en fallas"
description: "Una de las mejores épocas del año para conocer Valencia"
tags:  Barcelona,Peñíscola,Valencia
image:
  feature: valencia.jpg
  topPosition: 0px
bgContrast: dark
bgGradientOpacity: darker
syntaxHighlighter: no
---
#### Sabías que podéis recorrer la provincia de Valencia durante las fiestas de fallas, hay excelentes chollos disponibles!

- 4 días
- 2 ciudades
- Menos de € 100 !

[Alsa](https://www.alsa.es/-/fallas) posee ofertas desde € 5 para varios trayectos, entre ellos: Valencia. El mes de marzo es especial en Valencia debido a las <a href="https://es.wikipedia.org/wiki/Fallas_de_Valencia" target="_blank">fallas</a>! Que esperas?


<hr>
<div class="row">
  <div class="col-md-6 text-center">
    Barcelona - Peñíscola
    <hr>
    <br>
    <img style="width: 20%;" src="{{ site.baseurl_icons_img }}bus.svg" />
    <div>
      <a target="_blank" href="http://www.goeuro.es/autobuses_desde_barcelona_a_peniscola">Ver disponibilidad</a>
    </div>
  </div>
  <div class="col-md-6">
    <img style="width: 100%; border-radius: 30px;" src="{{ site.baseurl_routes_img }}barcelona_peniscola.png" />
  </div>
</div>
<hr>
<div class="text-center">
Peñíscola
<a href="http://www.booking.com/city/es/peniscola.html?aid=956648">Ver disponibilidad</a>
</div>
<hr>
<div class="row">
  <div class="col-md-6 text-center">
    Peñíscola - Valencia
    <hr>
    <br>
    <img style="width: 20%;" src="{{ site.baseurl_icons_img }}train.svg" />
    <div>
      <a target="_blank" href="http://www.goeuro.es/trenes_desde_peniscola_a_valencia">Ver disponibilidad</a>
    </div>
  </div>
  <div class="col-md-6">
    <img style="width: 100%; border-radius: 30px;" src="{{ site.baseurl_routes_img }}peniscola_valencia.png" />
  </div>
</div>
<hr>
<div class="text-center">
Valencia
<a href="http://www.booking.com/city/es/valencia.html?aid=956648">Ver disponibilidad</a>
</div>
<hr>
<div class="row">
  <div class="col-md-6 text-center">
    Valencia - Barcelona
    <hr>
    <br>
    <img style="width: 20%;" src="{{ site.baseurl_icons_img }}bus.svg" />
    <div>
      <a target="_blank" href="http://www.goeuro.es/autobuses_desde_valencia_a_barcelona">Ver disponibilidad</a>
    </div>
  </div>
  <div class="col-md-6">
    <img style="width: 100%; border-radius: 30px;" src="{{ site.baseurl_routes_img }}barcelona_peniscola.png" />
  </div>
</div>

Si te ha gustado nuestra idea, dejanos un comentario!
